...
import {
  ...
  text1,
  text2,
} from "./LaundryTransactionScreenStyle";
import GetApi from "../../api/GetApi";
import moment from "moment/moment";
import AsyncStorage from "@react-native-async-storage/async-storage";


const LaundryTransactionScreen = ({ navigation, props }) => {
  ...
  const getTransactionData = async () => {
    ...
    try {
      await GetApi.useFetch(
        "GET",
        "",
        `/transaction/GetTransactionDataLaundry.php?laundry_id=${account}`
      ).then((res) => {
...
export default LaundryTransactionScreen;
